const request = require('request'); //發送HTTP請求
const crypto = require('crypto'); //產生亂數
const Promise = require('bluebird'); //增強promise功能
...
class LineNotify {
  ...
  authDirect() { //產生LINE Notify的授權網址
    return (req, res) => {
      ...
      const url = this.secure_auth_url(state); //取得與LINE Notify相關參數的網址，將state與nonce加入Line Notify參數的網址，提高安全性
      return res.redirect(url); //傳送HTTP狀態碼302與LINE Server登入的跳轉網址給使用者的瀏覽器
    };
  }
      ...
      this.issue_line_access_token(code).then((token_response) => { //取得Access Token
        ...
        this.checkToken(token_response).then(
          result =>{
            ...
            req.session.notify_info = { //儲存LINE使用者資訊
